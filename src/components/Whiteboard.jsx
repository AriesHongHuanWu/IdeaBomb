    ...
    return (
        <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'white', borderRadius: 12 }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ fontSize: '3rem', fontWeight: 'bold', fontFamily: 'monospace', color: '#333' }}>
                    {format(timeLeft)}
                </div>
    ...
    return (
        ...
        <div ref={containerRef} onPointerDown={handlePointerDown} onPointerMove={(e) => { handlePointerMove(e); handleGlobalMouseMove(e) }} onPointerUp={handlePointerUp} onContextMenu={handleBgContextMenu} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ width: '100%', height: '100%', overflow: 'hidden', background: 'transparent', position: 'relative', touchAction: 'none', cursor: connectMode ? 'crosshair' : (isDraggingCanvas ? 'grabbing' : 'default') }}>
            ...
            <motion.div className="glass-panel" style={{ position: 'absolute', bottom: useMediaQuery('(max-width: 768px)') ? 85 : 30, left: '50%', x: '-50%', padding: '12px 24px', display: 'flex', gap: 20, borderRadius: 24, zIndex: 100, pointerEvents: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.1)', maxWidth: '90vw', overflowX: 'auto', background: theme?.cardBg, border: `1px solid ${theme?.border}` }} initial={{ y: 100 }} animate={{ y: 0 }}>
                <ToolBtn icon={<FiType />} label={t('sticky')} onClick={() => addCenteredNode('Note')} theme={theme} />
                <ToolBtn icon={<FiCheckSquare />} label={t('todo')} onClick={() => addCenteredNode('Todo')} theme={theme} />
                <ToolBtn icon={<FiCalendar />} label={t('calendar')} onClick={() => addCenteredNode('Calendar')} theme={theme} />
                <ToolBtn icon={<FiImage />} label={t('image')} onClick={() => addCenteredNode('Image')} theme={theme} />
                <div style={{ width: 1, height: 40, background: '#e0e0e0', margin: '0 5px' }}></div>
                <ToolBtn icon={<FiTarget />} label={t('magnet') || 'Magnet'} active={magnetMode} onClick={() => setMagnetMode(!magnetMode)} theme={theme} />
                <ToolBtn icon={<FiGrid />} label={t('toolbox') || 'Toolbox'} active={toolboxOpen} onClick={() => setToolboxOpen(!toolboxOpen)} theme={theme} />
                <div style={{ width: 1, height: 40, background: '#e0e0e0', margin: '0 5px' }}></div>
                <ToolBtn icon={<FiMaximize2 />} label={t('canvasSize')} onClick={() => setShowCanvasSetup(true)} theme={theme} />
                <div style={{ width: 1, height: 40, background: '#e0e0e0', margin: '0 5px' }}></div>
                <ToolBtn icon={<FiLayout />} label={t('autoArrange') || 'Auto Arrange'} onClick={autoArrange} theme={theme} />
            </motion.div>
