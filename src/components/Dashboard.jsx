                            ...
                            >
                                {modalConfig.type === 'confirm' ? (
                                    ...
                                    <>
                                        ...
                                        {/* Notification Toggle */}
                                        <div style={{ marginBottom: 30 }}>
                                            <div style={{ fontSize: '0.9rem', fontWeight: 600, color: '#5f6368', marginBottom: 10 }}>🔔 Notifications</div>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: theme.bg, borderRadius: 8, border: `1px solid ${theme.border}` }}>
                                                ...
                                                {notifPermission !== 'granted' && notifPermission !== 'denied' && (
                                                    <button
                                                        ...
                                                        style={{ padding: '6px 14px', borderRadius: 6, border: 'none', background: '#1a73e8', color: 'white', cursor: 'pointer', fontWeight: 500, fontSize: '0.85rem' }}
                                                    >
                                                        Enable
                                                    </button>
                                                )}
                                                {notifPermission === 'denied' && (
                                                    <span style={{ fontSize: '0.8rem', color: '#999' }}>Check browser settings</span>
                                                )}
                                            </div>
                                        </div>
                ...
                {showNotifModal && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
                        <motion.div
                            ...
                            style={{
                                background: theme.cardBg, padding: 30, borderRadius: 20, maxWidth: 400, width: '90%',
                                boxShadow: '0 25px 50px rgba(0,0,0,0.25)', textAlign: 'center'
                            }}
                        >
                            <div style={{ fontSize: '3rem', marginBottom: 15 }}>🔔</div>
                            <h2 style={{ margin: '0 0 10px 0', fontSize: '1.5rem', color: theme.textPrim }}>Stay in the Loop!</h2>
                            <p style={{ margin: '0 0 25px 0', color: theme.text, opacity: 0.8, lineHeight: 1.6 }}>
                                Enable notifications to get real-time updates when your team sends messages in shared whiteboards.
                            </p>
                            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                                <button
                                    onClick={() => { setShowNotifModal(false); localStorage.setItem('hasSeenNotifModal', 'true') }}
                                    style={{ padding: '12px 24px', borderRadius: 10, border: `1px solid ${theme.border}`, background: 'transparent', color: theme.text, cursor: 'pointer', fontWeight: 500 }}
                                >
                                    Maybe Later
                                </button>
                                <button
                                    onClick={async () => {
                                        ...
                                        localStorage.setItem('hasSeenNotifModal', 'true')
                                        await handleEnableNotifications()
                                    }}
                                    style={{ padding: '12px 24px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', cursor: 'pointer', fontWeight: 600, boxShadow: '0 4px 15px rgba(102, 126, 234, 0.4)' }}
                                >
