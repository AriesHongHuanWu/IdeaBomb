...
3. "create_calendar_plan": { "action": "create_calendar_plan", "events": { "1": "Event 1", "5": "Event 2" } }
...
[
  ...
  { "action": "create_node", "nodeType": "Todo", "content": "Tasks" },
  { "action": "create_node", "nodeType": "Calendar", "content": "Timeline" }
]
\`\`\`

If no action is needed, just reply with text. Be concise.
`
...
export default function ChatInterface({ onAction }) {
  ...
  const [messages, setMessages] = useState([{ role: 'model', text: 'Hi! I can help manage your board. Try "Create a study plan for React".' }])
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          if (Array.isArray(parsed)) { onAction(parsed); setMessages(p => [...p, { role: 'model', text: `I've created ${parsed.length} items for you.` }]) }
          else if (parsed.action) { onAction([parsed]); setMessages(p => [...p, { role: 'model', text: "I've added that to your whiteboard." }]) }
        } catch (e) { console.error("Parse error", e); setMessages(p => [...p, { role: 'model', text: text }]) }
      } else { setMessages(p => [...p, { role: 'model', text }]) }
    } catch (error) { setMessages(p => [...p, { role: 'model', text: `Error: ${error.message}` }]) }
  ...
  return (
    <>
      ...
      <div style={{ position: 'absolute', bottom: 30, right: 30, zIndex: 200, display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
        <AnimatePresence>
          {isOpen && (
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="glass-panel" style={{ width: 350, height: 500, marginBottom: 20, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'rgba(255, 255, 255, 0.85)' }}>
              <div style={{ padding: '15px 20px', borderBottom: '1px solid rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 600 }}><FiCpu style={{ color: 'var(--primary)' }} /> Gemini Assistant</div><button onClick={() => setIsOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}><FiX /></button></div>
              <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 15 }}>
                {messages.map((m, i) => (<div key={i} style={{ alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', background: m.role === 'user' ? 'var(--primary)' : 'rgba(255,255,255,0.7)', color: m.role === 'user' ? 'white' : 'var(--text-main)', padding: '10px 15px', borderRadius: '12px', maxWidth: '85%', fontSize: '0.9rem', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}><ReactMarkdown>{m.text}</ReactMarkdown></div>))}
              ...
              <div style={{ padding: 15, borderTop: '1px solid rgba(0,0,0,0.05)', display: 'flex', gap: 10 }}>
                ...
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Type or speak..." style={{ flex: 1, border: 'none', background: 'rgba(0,0,0,0.05)', borderRadius: 20, padding: '0 15px', outline: 'none' }} />
...
function PrivacyInput({ apiKey, setApiKey, isOpen }) {
  ...
  return (<div style={{ position: 'absolute', top: 20, right: 20, zIndex: 300 }}><input type="password" placeholder="Gemini API Key" onChange={(e) => setApiKey(e.target.value)} className="glass-panel" style={{ padding: '5px 10px', width: 150, fontSize: '0.8rem' }} /></div>)
}
