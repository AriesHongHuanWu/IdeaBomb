...
export default function ChatInterface({ onAction }) {
  ...
  const toggleListening = () => {
    ...
    recognition.lang = 'en-US' // Or 'zh-TW' if user wants Chinese voice
...
function PrivacyInput({ apiKey, setApiKey, isOpen }) {
  ...
  return (
    <div style={{ position: 'absolute', top: 20, right: 20, zIndex: 300 }}>
      <input type="password" placeholder="API Key" onChange={(e) => setApiKey(e.target.value)} className="glass-panel" style={{ padding: '5px 10px', width: 150, fontSize: '0.8rem' }} />
    </div>
  )
}
